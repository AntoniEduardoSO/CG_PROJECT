...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            if (Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out RaycastHit hit2, 3f))
            {
                if (hit2.transform.CompareTag("Inspectable"))
                {
                    EnterInspectionMode(hit2.transform.gameObject);
                }
            }
        }
    }
    ...
    private void EnterInspectionMode(GameObject obj)
    {
        playerCanMove = false;
        isInspecting = true;
        currentObject = obj;
        cameraCanMove = false;  // Desabilita o movimento da câmera
        Cursor.lockState = CursorLockMode.None;
        Cursor.visible = false;
        currentObject.GetComponent<Rigidbody>().isKinematic = true; // Congelar física
    }

    private void ExitInspectionMode()
    {
        isInspecting = false;
        cameraCanMove = true;  // Habilita novamente o movimento da câmera
        Cursor.lockState = CursorLockMode.Locked;
        Cursor.visible = false;
        playerCanMove = true;
    ...
    private void RotateObject()
    {
        if (currentObject != null)
        {
            ...
            float mouseX = Input.GetAxis("Mouse X") * rotationSpeed * Time.deltaTime * mouseSensitivity;
            float mouseY = Input.GetAxis("Mouse Y") * rotationSpeed * Time.deltaTime * mouseSensitivity;
